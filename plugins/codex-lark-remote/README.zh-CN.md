...
这个仓库是 Lark Remote Codex 插件的开发工作区。真正可安装、可提交市场审核的
插件目录是 `plugins/codex-lark-remote/`；仓库根目录只放测试、发布说明和开发脚本，
避免它们进入 marketplace 扫描面。
...
marketplace，里面只有一个指向 `./plugins/codex-lark-remote` 的插件条目。
...
marketplace 提交面是嵌套插件目录。开发测试、发布脚本和生成产物不要放进
`plugins/codex-lark-remote/`。
