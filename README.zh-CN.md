...
7. 按平台提示开通消息接收、发送/回复消息、卡片交互回调相关权限，然后发布或启用应用。

卡片按钮依赖 `card.action.trigger`。如果能收到文本消息但点击卡片没有反应，优先检查：
应用是否已发布、事件订阅是否包含 `card.action.trigger`、回调配置是否仍是长连接、以及权限变更后是否重新发布。
...
从飞书向机器人发送 `whoami`，再把返回的 `senderId` 加进去。
